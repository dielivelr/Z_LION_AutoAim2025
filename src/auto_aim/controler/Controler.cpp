...
namespace rm
{
	...
	{
		// 一旦设定模式,进入锁定状态
		if (state != StrategyPlanner::STATE::defult) this->lock = true;
		...
		const Eigen::VectorXf& pred_state,const float& car_yaw, const float& car_pitch, const float& car_yaw_speed)
	{
		...
		switch (state)
		{
		case rm::StrategyPlanner::STATE::defult:
			return TransControler();
		case rm::StrategyPlanner::STATE::shoot_track:
		{
		...
		break;
		case rm::StrategyPlanner::STATE::shoot_middle:
		{
		...
		break;
		case rm::StrategyPlanner::STATE::shoot_hero:
		{
			if (last_iteration) { // 处于迭代,仅仅计算最后一次目标的延迟
		...
		break;
		case rm::StrategyPlanner::STATE::shoot_precise:
		{
			if (last_iteration) { // 处于迭代,仅仅计算最后一次目标的延迟
		...
		break;
		case rm::StrategyPlanner::STATE::shoot_translation:
		{ 
			// 选取击打目标
		...
		break;
		case rm::StrategyPlanner::STATE::shoot_vague:
		{
		...
		// 处于预瞄状态,则反向增幅 / 不增幅
		if (state == rm::StrategyPlanner::STATE::shoot_track) { // 只有在跟随击打下才使用选择旋转增幅
	...
	void Controler::clear()
	{
		...
		if(!lock)
			this->state = StrategyPlanner::STATE::defult;
